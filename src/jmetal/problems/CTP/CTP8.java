...
/**
 * Class representing problem CTP8
 */
public class CTP8 extends Problem {
    /**
     * Creates an  CTP8 problem (10 variables and 2 objectives)
     *
     * @param solutionType The solution type must "Real" or "BinaryReal".
     */
    public CTP8(String solutionType) throws ClassNotFoundException {
        this(solutionType, 4, 2);
    } // CTP8

    /**
     * Creates an CTP8 problem instance
     *
     * @param numberOfVariables  Number of variables
     * @param numberOfObjectives Number of objective functions
     * @param solutionType       The solution type must "Real" or "BinaryReal".
     */
    public CTP8(String solutionType,
    ...
    public void evaluateConstraints(Solution solution) throws JMException {
        ...
        double theta = -0.05 * Math.PI, a = 40, b = 2, c = 1, d = 6, e = 0;
        double f1 = solution.getObjective(0);
        double f2 = solution.getObjective(1);
        double[] cons = new double[2];
        cons[0] = Math.cos(theta) * (f2 - e) - Math.sin(theta) * f1
        ...
        cons[1] = Math.cos(theta) * (f2 - e) - Math.sin(theta) * f1
                - a * Math.pow(Math.abs(Math.sin(b * Math.PI * Math.pow(Math.sin(theta) * (f2 - e)
                + Math.cos(theta) * f1, c))), d);
        ...
        for (int i = 0; i < 2; i++) {
            if (cons[i] < 0.0) {
                total = total - cons[i];
                number++;
                solution.setConstraint(i, -cons[i]);
            }else {
