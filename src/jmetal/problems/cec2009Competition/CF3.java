...
//	f[1] = 1.0 - x[0]*x[0] + 2.0*(4.0*sum2 - 2.0*prod2 + 2.0) / (double)count2;
    f[0] = x[0]	           + 2.0*(4.0*sum1) / (double)count1;
	f[1] = 1.0 - x[0]*x[0] + 2.0*(4.0*sum2) / (double)count2;
	// constraint evaluation
    double constraint;
    constraint = f[1] + f[0]*f[0] - a*Math.sin(N*Math.PI*(f[0]*f[0]-f[1]+1.0)) - 1.0;
    double total = 0.0;
    int number = 0;
    if (constraint < 0.0) {
        total += -constraint;
