...
public class CF7 extends Problem {
  ...
  * @param solutionType The solution type must "Real" or "BinaryReal".
  */
  public CF7(String solutionType, Integer numberOfVariables) {
    numberOfVariables_  = numberOfVariables;
    numberOfObjectives_ =  2;
    numberOfConstraints_=  2;
    problemName_        = "CEC2009_CF7";
  ...
  public void evaluate(Solution solution) throws JMException {
    ...
    constraints[0] = x[1]-Math.sin(6.0*x[0]*Math.PI+2.0*Math.PI/numberOfVariables_) - MYSIGN((x[0]-0.5)*(1.0-x[0]))*Math.sqrt(Math.abs((x[0]-0.5)*(1.0-x[0])));
	constraints[1] = x[3]-Math.sin(6.0*x[0]*Math.PI+4.0*Math.PI/numberOfVariables_) - MYSIGN(0.25*Math.sqrt(1-x[0])-0.5*(1.0-x[0]))*Math.sqrt(Math.abs(0.25*Math.sqrt(1-x[0])-0.5*(1.0-x[0])));
	double total = 0.0;
	int number = 0;
