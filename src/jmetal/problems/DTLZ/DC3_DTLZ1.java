 ...
 */
public class DC3_DTLZ1 extends Problem {   
 /** 
  * Creates a default C1_DTLZ1 problem (7 variables and 3 objectives)
  * @param solutionType The solution type must "Real" or "BinaryReal". 
  */
  public DC3_DTLZ1(String solutionType) throws ClassNotFoundException {
  ...
  * @param solutionType The solution type must "Real" or "BinaryReal". 
  */
  public DC3_DTLZ1(String solutionType, Integer numberOfVariables, 
  		         Integer numberOfObjectives) throws ClassNotFoundException {
    numberOfVariables_   = numberOfVariables.intValue();
    numberOfObjectives_  = numberOfObjectives.intValue();
    numberOfConstraints_ = numberOfObjectives.intValue()+1;
    problemName_         = "DC3_DTLZ1";
  ...
  */    
	public void evaluate(Solution solution) throws JMException {
		...
		double [] constraints=new double[numberOfObjectives_+1];
		double a=5.0,b=0.5;
		for(int i=0;i<numberOfObjectives_;i++) {
			constraints[i]=Math.cos(a*Math.PI*x[i])-b;
		}
		constraints[numberOfObjectives_]=Math.cos(a*Math.PI*g)-b;
		double total = 0.0;
		int number = 0;
		for(int i=0;i<numberOfObjectives_+1;i++) {
			if (constraints[i] < 0.0) {
				total=total-constraints[i];
				number++;
				solution.setConstraint(i, -constraints[i]);
			}else {
