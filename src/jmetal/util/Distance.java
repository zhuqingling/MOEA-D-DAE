...
import jmetal.util.comparators.PseudoObjectiveComparator;
...
public class Distance {
  ...
  public double [][] SDE_distanceMatrix(SolutionSet solutionSet) {
	    Solution solutionI, solutionJ;
	    ...
	    for (int i = 0; i < solutionSet.size(); i++){
	      ...
	      for (int j = 0; j < solutionSet.size(); j++){
	        ...
	        {
	        	distance[i][j] = this.SDE_distanceBetweenObjectives(solutionI,solutionJ);
	        }
	      } // for
	    } // for        
  ...
  public double SDE_distanceBetweenObjectives(Solution solutionI, Solution solutionJ){
	    /* euclidian distance
	    double diff;    //Auxiliar var
	    double distance = 0.0;
	    //-> Calculate the euclidean distance
	    ...
	    */ 
	    ...
	    double diff_w;    //Auxiliar var
	    double distance_w = 0.0;
	    //-> Calculate the euclidean distance
	    for (int nObj = 0; nObj < solutionI.getNumberOfObjectives();nObj++){
	    	if(solutionI.getObjective(nObj)<solutionJ.getObjective(nObj))
	    	{
				diff_b = solutionI.getObjective(nObj) - solutionJ.getObjective(nObj);
				distance_b += Math.pow(diff_b,2.0);
	    	}
	    	else
	    	{
	    		diff_w = solutionI.getObjective(nObj) - solutionJ.getObjective(nObj);
				distance_w += Math.pow(diff_w,2.0);
	    	}
	    } // for   
	  ...
	  double diff = solutionI.getObjective(0)-solutionJ.getObjective(0);    //Auxiliar var
	    //double distance = 0.0;
	    //-> Calculate the euclidean distance
	    for (int nObj = 1; nObj < solutionI.getNumberOfObjectives();nObj++){
	    	if(diff < solutionI.getObjective(nObj)-solutionJ.getObjective(nObj))
	    	{
	    		diff = solutionI.getObjective(nObj) - solutionJ.getObjective(nObj);
	    		//distance += Math.pow(diff,2.0);
	    	}
  ...
  public void crowdingDistanceAssignment(SolutionSet solutionSet, int nObjs) {
    ...
    for (int i = 0; i<nObjs; i++) {
      ...
      front.get(size-1).setFitness(Double.POSITIVE_INFINITY);
      
      for (int j = 1; j < size-1; j++) {
        distance = front.get(j+1).getObjective(i) - front.get(j-1).getObjective(i);
        double fitness = 0;
        fitness = front.get(j).getFitness()+Math.pow(distance, 2.0);
		front.get(j).setFitness(fitness);
        distance = distance / (objetiveMaxn - objetiveMinn + Double.MIN_VALUE);
  ...
  public void crowdingDistanceAssignmentforAP(SolutionSet solutionSet, int nObjs) {
	    int size = solutionSet.size();
	    ...
	    for (int i = 0; i<nObjs; i++) {
	      // Sort the population by Obj n
	    	try {
	      front.sort(new PseudoObjectiveComparator(i));
	    	}catch(Exception e) {
	    		System.out.println("error");
	    	}
	      objetiveMinn = front.get(0).getPseudoObjective(i);
	      objetiveMaxn = front.get(front.size()-1).getPseudoObjective(i);
	      ...
	      for (int j = 1; j < size-1; j++) {
	        distance = front.get(j+1).getPseudoObjective(i) - front.get(j-1).getPseudoObjective(i);
	        double fitness = 0;
	        fitness = front.get(j).getFitness()+Math.pow(distance, 2.0);
			front.get(j).setFitness(fitness);
	        distance = distance / (objetiveMaxn - objetiveMinn + Double.MIN_VALUE);
  ...
  public void crowdingDistanceAssignment_BCE(SolutionSet solutionSet, int nObjs) {
	    int size = solutionSet.size();
	    ...
	    double [][] PF = solutionSet.writeObjectivesToMatrix();//N*m
	    // STEP 1. Obtain the maximum and minimum values of the Pareto front
	    double [] maximumValue = utils_.getMaximumValues(PF, nObjs);
	    double [] minimumValue = utils_.getMinimumValues(PF, nObjs);
	    
	    // STEP 2. Get the normalized front and true Pareto fronts
	    normalizedFront = utils_.getNormalizedFront(PF,maximumValue,minimumValue);//N*m
	    
	    double [][] d_m = distanceMatrix_BCE(normalizedFront);
//	    double [][] d_t = new double[d_m.length][d_m[0].length];
//	    for(int i=0;i<d_m.length;i++){
//	    	for(int j=0;j<d_m[0].length;j++){
//	    		d_t[i][j] = d_m[i][j];
//	    	}
//	    }
	    int k = 2;
	    ...
	    for (int i = 0; i < d_m.length; i++) {
	      Arrays.sort(d_m[i]);
	      sum_d3 = sum_d3 + d_m[i][k]; // Calcule de D(i) distance         
	    }// for
	    double r = sum_d3/(double)d_m.length;
	    for(int i=0;i<d_m.length;i++){
	    	double R = 1.0;
	    	for(int j=0; j<d_m[0].length;j++){
	    		if(d_m[i][j] <= r && d_m[i][j]>0){
	    			R = R * d_m[i][j]/r;
	    		}
	    	}
	    	solutionSet.get(i).setCrowdingDistance(R);
	    }
	  } // crowdingDistanceAssing_BCE
  
  public double [][] distanceMatrix_BCE(double [][] solutionSet) {
	    double [] solutionI, solutionJ;

	    //The matrix of distances
	    double [][] distance = new double [solutionSet.length][solutionSet.length];
	    //-> Calculate the distances
	    for (int i = 0; i < solutionSet.length; i++){
	      distance[i][i] = 0.0;
	      solutionI = solutionSet[i];
	      for (int j = i + 1; j < solutionSet.length; j++){
	        solutionJ = solutionSet[j];
	        distance[i][j] = utils_.distance(solutionI,solutionJ);
	        distance[j][i] = distance[i][j];
	      } // for
	    } // for        
