...
//    distance_.crowdingDistanceAssignment_BCE(this,objectives_);
...
//    	distance_.crowdingDistanceAssignment_BCE(this,objectives_);
      distance_.crowdingDistanceAssignment(this,objectives_);
//    	remove(indexBest(crowdingDistance_));
      int index=0;
      double worstKnown=solutionsList_.get(0).getCrowdingDistance();
      for (i = 1; i < solutionsList_.size(); i++) {
        if (worstKnown>solutionsList_.get(i).getCrowdingDistance()) {
          index = i;
          worstKnown = solutionsList_.get(i).getCrowdingDistance();
        }
      }
      remove(index);
    }        
  ...
  public boolean add_constraint(Solution solution){
	    int flag = 0;
	    int i = 0;
	    Solution aux; //Store an solution temporally
	    while (i < solutionsList_.size()){
