...
public class Distance1 {
  ...
  public double [][] distanceMatrix_BEC(double [][] solutionSet) {
  ...
  public void crowdingDistanceAssignment_BEC(SolutionSet solutionSet, int nObjs) {
	    int size = solutionSet.size();
	    if(size < 100)
	    ...
	    double [][] d_m = distanceMatrix_BEC(normalizedFront);
...
//	    int k = (int) Math.sqrt(size/2.0);
	    ...
	    for(int i=0;i<d_m.length;i++){
	    	double R = 1.0;
	    	for(int j=0; j<d_m[i].length;j++){
	    		if(d_m[i][j] < r && d_m[i][j] >0){
	    			R = R * d_m[i][j]/r;
	    		}
	    	}
	    	solutionSet.get(i).setCrowdingDistance(1.0-R);
	    }
